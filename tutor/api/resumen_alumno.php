...
function validar_tutor() {
    if (empty($_SESSION['active']) || ($_SESSION['rol'] ?? '') !== 'Padre') {
        json_response(false, 'No autorizado');
    }

    $tutor_id = (int)($_SESSION['idTutor'] ?? 0);
    if ($tutor_id <= 0) {
        $idUsuario = (int)($_SESSION['usuario_id'] ?? $_SESSION['idUsuario'] ?? 0);
        if ($idUsuario > 0) {
            $resTutor = buscar_datos("SELECT idTutor FROM tutor WHERE idUsuario = $idUsuario AND estado = 'Activo' LIMIT 1");
            if ($resTutor) {
                $tutor_id = (int)$resTutor[0]['idTutor'];
            }
        }
    }

    if ($tutor_id <= 0) json_response(false, 'Tutor no identificado');
    return $tutor_id;
}
...
function tabla_existe($nombreTabla) {
    $res = buscar_datos("SHOW TABLES LIKE '$nombreTabla'");
    return !empty($res);
}
...
function calcular_proceso_materia($idMatricula, $idAulaMateria, $periodos, $matInfo = []) {
    ...
    // Nota final desde nota_final_materia cuando exista la tabla; si no, se calcula on-the-fly
    $nf = null;
    if (tabla_existe('nota_final_materia')) {
        $resNF = buscar_datos("
