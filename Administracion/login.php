...
function obtenerRutaDeCookie() {
    if (empty($_COOKIE['ultima_ruta_admin'])) {
        return false;
    }
    return validarRutaSegura($_COOKIE['ultima_ruta_admin']);
}
...
$rol = buscar_datos("SELECT * FROM usuarios u 
JOIN rol r ON u.idRol = r.idRol
WHERE u.correo = '" . $correo . "'");

if ($rol === false || count($rol) === 0 || !in_array($rol[0]['rol'], ['Director', 'Secretario', 'Evaluador'], true)) {
    if ($isAjax) {
        echo json_encode(['success' => false, 'message' => '¡Acceso denegado! Solo los encargados administrativos pueden ingresar.']);
        exit;
    }
    header('Location: index.php?error=1');
    exit;
...
if ($datos !== false && count($datos) > 0) {
    ...
    if (password_verify($password, $passwordStored) || $passwordStored === $password) {
        ...
        //Validar que la ruta guardada sea de Administracion
        $destino = obtenerRutaDeCookie() ?: 'menu.php';
        $rutaCookieAdmin = obtenerRutaDeCookie();
        if ($rutaCookieAdmin && strpos($rutaCookieAdmin, '/Administracion/') !== false) {
            $destino = $rutaCookieAdmin;
        }
