<?php
    ...
    // Validar ID 
    if (!$id) {
        $response['msg'] = 'Identificador de docente no válido.';
        echo json_encode($response);
        exit;
    }

    // Validar campos obligatorios 
    if (empty($nombres) || empty($apellidos) || empty($estado)) {
    ...
    $sql_check_ci = "SELECT idDocente FROM docente WHERE cedula = '$cedula' AND idDocente != $id";
    ...
    $sql_dup = "SELECT idDocente FROM docente
                WHERE nombres = '$nombres' AND apellidos = '$apellidos' AND idDocente != $id";
